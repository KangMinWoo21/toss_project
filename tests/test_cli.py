...
class CliTests(unittest.TestCase):
    ...
    def test_ml_baseline_model_training_cli_writes_reports(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            self._write_ml_data_readiness_sources(root)
            ...
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertIn("training_status  paper_only_baseline_trained", completed.stdout)
            self.assertIn("trading_allowed  False", completed.stdout)
            self.assertIn("production_effect  none", completed.stdout)
            self.assertTrue(csv_output.exists())
            self.assertTrue(md_output.exists())
    ...
    def test_ml_financial_feature_schema_plan_cli_writes_reports(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertIn("schema_plan_status  PASS", completed.stdout)
            self.assertIn("fetch_allowed_now  False", completed.stdout)
            self.assertIn("training_allowed_now  False", completed.stdout)
            self.assertIn("trading_allowed  False", completed.stdout)
            self.assertIn("production_effect  none", completed.stdout)
    ...
    def test_ml_sentiment_scoring_plan_cli_writes_reports(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            ...
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertIn("sentiment_plan_status  PASS", completed.stdout)
            self.assertIn("model_version  rule_lexicon_v1", completed.stdout)
            self.assertIn("training_allowed_now  False", completed.stdout)
            self.assertIn("feature_added_to_training  False", completed.stdout)
            self.assertIn("trading_allowed  False", completed.stdout)
            self.assertIn("production_effect  none", completed.stdout)
    ...
    def test_ml_model_observation_status_cli_writes_reports(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            reports = root / "data" / "reports"
            ...
            self.assertEqual(completed.returncode, 0, completed.stderr)
            self.assertIn("observation_status  paper_only_observation_started", completed.stdout)
            self.assertIn("sufficient_observation_months  False", completed.stdout)
            self.assertIn("candidate_promotion  False", completed.stdout)
            self.assertIn("trading_allowed  False", completed.stdout)
            self.assertIn("production_effect  none", completed.stdout)
            self.assertTrue(csv_output.exists())
            self.assertTrue(md_output.exists())
    ...
    def test_monthly_backtest_help_includes_deep_drawdown_guard_options(self):
        ...
        self.assertIn("--market-beta-proxy-buyable-only", completed.stdout)
    ...
    def test_monthly_attribution_help_includes_stress_and_output_options(self):
        ...
        self.assertIn("--benchmark-selection-summary-output", completed.stdout)
        self.assertIn("--market-beta-proxy-max-exposure", completed.stdout)
        self.assertIn("--market-beta-proxy-neutral-breadth-max-exposure", completed.stdout)
        self.assertIn("--market-beta-proxy-neutral-loss-guard-max-exposure", completed.stdout)
        self.assertIn("--market-beta-proxy-reversal-guard-max-exposure", completed.stdout)
    ...
    def test_monthly_attribution_cli_writes_recovery_summary_report(self):
        with TemporaryDirectory() as temp_dir:
            ...
            benchmark_selection_summary_output = root / "benchmark_selection_summary.csv"
            ...
            completed = self._run_backtester_in_cwd(
                ...
                [
                    ...
                    str(benchmark_selection_summary_output),
            ...
            if benchmark_selection_summary_output.exists():
                with benchmark_selection_summary_output.open(encoding="utf-8") as f:
                    benchmark_selection_summary_rows = list(csv.DictReader(f))
            else:
        ...
        self.assertIn("benchmark_selection_summary_report", completed.stdout)
        ...
        self.assertTrue(benchmark_selection_summary_rows)
        ...
        self.assertEqual(benchmark_selection_summary_rows[0]["scenario"], "walk_forward_unit")
        ...
        self.assertIn("low_liquidity_missed_winner_delta_share", benchmark_selection_summary_rows[0])
