...
class ProductionReadinessTests(unittest.TestCase):
    ...
    def test_risk_report_missing_data_gate_rows_blocks_readiness(self):
        with TemporaryDirectory() as temp_dir:
            risk = Path(temp_dir) / "risk.csv"
            risk.write_text(
                "name,status,detail\n"
                "kill_switch,PASS,not present\n"
                "orders,PASS,valid\n",
                encoding="utf-8",
            )
        ...
        self.assertIn("market_data_freshness", checks[0].detail)

    def test_risk_report_missing_required_gate_detail_blocks_readiness(self):
        with TemporaryDirectory() as temp_dir:
            risk = Path(temp_dir) / "risk.csv"
            risk.write_text(
                "name,status,detail\n"
                "point_in_time_universe,PASS,\n"
    ...
    def test_performance_report_missing_required_detail_blocks_readiness(self):
        with TemporaryDirectory() as temp_dir:
            performance = Path(temp_dir) / "performance.csv"
            performance.write_text(
                "name,status,detail\n"
                "required_scenarios,PASS,all required scenarios pass\n"
                "required_excess,PASS,\n"
    ...
    def test_validation_failure_report_blocks_unsafe_live_action(self):
        with TemporaryDirectory() as temp_dir:
            ...
            failures.write_text(
                "name,category,reason,severity,failed_metric,metric_value,threshold,suggested_action,parameter_hints\n"
                "stress,stress,max_drawdown_breach,BLOCK,max_drawdown_pct,-28,-25,LIVE_ORDER_AFTER_REVIEW,keep paper-only\n",
                encoding="utf-8",
            )
    ...
    def test_validation_failure_report_missing_required_columns_blocks_readiness(self):
        ...
        failure_checks = [check for check in checks if check.name == "validation_failure_actions"]
        self.assertEqual(failure_checks[0].status, "BLOCK")
        self.assertIn("missing_required_columns", failure_checks[0].detail)
    ...
    def test_validation_remediation_report_blocks_unsafe_live_parameter_hints(self):
        with TemporaryDirectory() as temp_dir:
            remediation = Path(temp_dir) / "monthly_validation_remediation.csv"
            remediation.write_text(
                "priority,suggested_action,failure_count,blocked_count,affected_categories,affected_scenarios,failed_metrics,worst_metric_value,parameter_hints,next_experiment\n"
                "P2,IMPROVE_WEAK_WINDOW_DEFENSE,1,1,regime,regime_sideways,excess_return_pct,-7.1648,fetch live data before review,Run attribution\n",
                encoding="utf-8",
            )
        ...
        remediation_checks = [check for check in checks if check.name == "validation_remediation"]
        self.assertEqual(remediation_checks[0].status, "BLOCK")
        self.assertIn("unsafe_parameter_hints", remediation_checks[0].detail)
    ...
    def test_validation_remediation_report_missing_required_columns_blocks_readiness(self):
        with TemporaryDirectory() as temp_dir:
            ...
            remediation.write_text(
                ...
                "P1,IMPROVE_WEAK_WINDOW_DEFENSE\n",
                encoding="utf-8",
            )
        ...
        remediation_checks = [check for check in checks if check.name == "validation_remediation"]
        self.assertEqual(remediation_checks[0].status, "BLOCK")
        self.assertIn("missing_required_columns", remediation_checks[0].detail)
        self.assertIn("affected_scenarios", remediation_checks[0].detail)
        self.assertIn("next_experiment", remediation_checks[0].detail)

    def test_validation_remediation_report_missing_required_values_blocks_readiness(self):
        with TemporaryDirectory() as temp_dir:
            remediation = Path(temp_dir) / "monthly_validation_remediation.csv"
            remediation.write_text(
                "priority,suggested_action,failure_count,blocked_count,affected_categories,affected_scenarios,failed_metrics,worst_metric_value,parameter_hints,next_experiment\n"
                "P1,IMPROVE_WEAK_WINDOW_DEFENSE,3,3,regime; walk_forward,,excess_return_pct,-7.1648,increase cash_buffer_weight,\n",
                encoding="utf-8",
            )
        ...
        remediation_checks = [check for check in checks if check.name == "validation_remediation"]
        self.assertEqual(remediation_checks[0].status, "BLOCK")
        self.assertIn("missing_required_values", remediation_checks[0].detail)
        self.assertIn("affected_scenarios", remediation_checks[0].detail)
        self.assertIn("next_experiment", remediation_checks[0].detail)
    ...
    def test_validation_sweep_plan_blocks_unsafe_live_risk_note(self):
        with TemporaryDirectory() as temp_dir:
            sweep = Path(temp_dir) / "monthly_validation_sweep_plan.csv"
            sweep.write_text(
                "priority,suggested_action,experiment_id,target_scenarios,cash_buffer_weight,min_train_positive_ratio,candidate_pool_size,max_position_weight,drawdown_guard_scale,market_volatility_min_scale,expected_effect,risk_note\n"
                "P1,IMPROVE_WEAK_WINDOW_DEFENSE,weak_defense_cash_05,regime_sideways,0.05,0.55,5,,,,Reduce weak-window exposure,Try live order after sweep\n",
                encoding="utf-8",
            )
        ...
        sweep_checks = [check for check in checks if check.name == "validation_sweep_plan"]
        self.assertEqual(sweep_checks[0].status, "BLOCK")
        self.assertIn("unsafe_risk_note", sweep_checks[0].detail)

    def test_validation_sweep_plan_blocks_unsafe_live_action(self):
        with TemporaryDirectory() as temp_dir:
            sweep = Path(temp_dir) / "monthly_validation_sweep_plan.csv"
            sweep.write_text(
                "priority,suggested_action,experiment_id,target_scenarios,cash_buffer_weight,min_train_positive_ratio,candidate_pool_size,max_position_weight,drawdown_guard_scale,market_volatility_min_scale,expected_effect,risk_note\n"
                "P2,LIVE_ORDER_AFTER_REVIEW,weak_defense_cash_05,regime_sideways,0.05,0.55,5,,,,Reduce weak-window exposure,Paper-only review\n",
                encoding="utf-8",
            )
    ...
    def test_validation_sweep_plan_blocks_unsafe_live_expected_effect(self):
        with TemporaryDirectory() as temp_dir:
            ...
            sweep.write_text(
                "priority,suggested_action,experiment_id,target_scenarios,cash_buffer_weight,min_train_positive_ratio,candidate_pool_size,max_position_weight,drawdown_guard_scale,market_volatility_min_scale,expected_effect,risk_note\n"
                "P1,IMPROVE_WEAK_WINDOW_DEFENSE,weak_defense_cash_05,regime_sideways,0.05,0.55,5,,,,Try live order after sweep,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        sweep_checks = [check for check in checks if check.name == "validation_sweep_plan"]
        self.assertEqual(sweep_checks[0].status, "BLOCK")
        self.assertIn("unsafe_expected_effect", sweep_checks[0].detail)

    def test_validation_sweep_plan_allows_no_trade_expected_effect(self):
        with TemporaryDirectory() as temp_dir:
            sweep = Path(temp_dir) / "monthly_validation_sweep_plan.csv"
            sweep.write_text(
                "priority,suggested_action,experiment_id,target_scenarios,cash_buffer_weight,min_train_positive_ratio,candidate_pool_size,max_position_weight,drawdown_guard_scale,market_volatility_min_scale,expected_effect,risk_note\n"
                "P1,IMPROVE_WEAK_WINDOW_DEFENSE,weak_defense_cash_05,regime_sideways,0.05,0.55,5,,,,Keep this as a no-trade gate unless validation improves,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        sweep_checks = [check for check in checks if check.name == "validation_sweep_plan"]
        self.assertEqual(sweep_checks[0].status, "WARN")
        self.assertNotIn("unsafe_expected_effect", sweep_checks[0].detail)
    ...
    def test_validation_sweep_results_blocks_unsafe_live_risk_note(self):
        with TemporaryDirectory() as temp_dir:
            ...
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--cash-buffer-weight 0.05,TARGET_ONLY,FULL_VALIDATION_REQUIRED,Run monthly-validate and compare,failed_required 1 -> 0,Try live order after sweep\n",
                encoding="utf-8",
            )
        ...
        self.assertIn("unsafe_risk_note", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_experiment_id(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "live_order_after_review,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--cash-buffer-weight 0.05,TARGET_ONLY,FULL_VALIDATION_REQUIRED,Run monthly-validate and compare,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        result_checks = [check for check in checks if check.name == "validation_sweep_results"]
        self.assertEqual(result_checks[0].status, "BLOCK")
        self.assertIn("unsafe_experiment_id", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_action(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,LIVE_ORDER_AFTER_REVIEW,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--cash-buffer-weight 0.05,TARGET_ONLY,FULL_VALIDATION_REQUIRED,Run monthly-validate and compare,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        result_checks = [check for check in checks if check.name == "validation_sweep_results"]
        self.assertEqual(result_checks[0].status, "BLOCK")
        self.assertIn("unsafe_suggested_action", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_candidate_args(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--live-order,TARGET_ONLY,FULL_VALIDATION_REQUIRED,Run monthly-validate and compare,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        result_checks = [check for check in checks if check.name == "validation_sweep_results"]
        self.assertEqual(result_checks[0].status, "BLOCK")
        self.assertIn("unsafe_candidate_validation_args", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_config_changes(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,enable_live_order=true,--cash-buffer-weight 0.05,TARGET_ONLY,FULL_VALIDATION_REQUIRED,Run monthly-validate and compare,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        result_checks = [check for check in checks if check.name == "validation_sweep_results"]
        self.assertEqual(result_checks[0].status, "BLOCK")
        self.assertIn("unsafe_config_changes", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_validation_scope(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--cash-buffer-weight 0.05,LIVE_TRADING,FULL_VALIDATION_REQUIRED,Run monthly-validate and compare,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        result_checks = [check for check in checks if check.name == "validation_sweep_results"]
        self.assertEqual(result_checks[0].status, "BLOCK")
        self.assertIn("unsafe_validation_scope", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_adoption_status(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--cash-buffer-weight 0.05,TARGET_ONLY,LIVE_APPROVED,Run monthly-validate and compare,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
        ...
        result_checks = [check for check in checks if check.name == "validation_sweep_results"]
        self.assertEqual(result_checks[0].status, "BLOCK")
        self.assertIn("unsafe_adoption_status", result_checks[0].detail)

    def test_validation_sweep_results_blocks_unsafe_live_adoption_requirements(self):
        with TemporaryDirectory() as temp_dir:
            results = Path(temp_dir) / "monthly_validation_sweep_results.csv"
            results.write_text(
                "experiment_id,suggested_action,status,target_scenarios,scenario_count,failed_required,baseline_failed_required,failed_delta,min_excess_return_pct,worst_drawdown_pct,trade_count,config_changes,candidate_validation_args,validation_scope,adoption_status,adoption_requirements,result_summary,risk_note\n"
                "weak_defense_cash_05,IMPROVE_WEAK_WINDOW_DEFENSE,IMPROVED,regime_sideways,1,0,1,-1,1.2,-8,1,cash_buffer_weight=0.05,--cash-buffer-weight 0.05,TARGET_ONLY,FULL_VALIDATION_REQUIRED,Place live order after review,failed_required 1 -> 0,Paper-only review\n",
                encoding="utf-8",
            )
    ...
    def test_validation_candidate_followup_blocks_unsafe_live_command(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            followup = root / "monthly_validation_candidate_followup.csv"
            followup.write_text(
                "priority_rank,experiment_id,status,adoption_status,failed_delta,candidate_validation_args,candidate_scenario_output,candidate_gate_output,comparison_output,delta_output,decision_output,validation_command,comparison_command,risk_note\n"
                "1,unsafe_live,IMPROVED,FULL_VALIDATION_REQUIRED,-1,--candidate-flag,candidate.csv,gate.csv,comparison.csv,delta.csv,missing_decision.csv,python -m backtester live-order,python -m backtester monthly-compare-validation,Plan only\n",
                encoding="utf-8",
            )
    ...
    def test_validation_candidate_followup_blocks_unsafe_live_experiment_id(self):
        with TemporaryDirectory() as temp_dir:
            ...
            followup.write_text(
                "priority_rank,experiment_id,status,adoption_status,failed_delta,candidate_validation_args,candidate_scenario_output,candidate_gate_output,comparison_output,delta_output,decision_output,validation_command,comparison_command,risk_note\n"
                "1,live_order_after_review,IMPROVED,FULL_VALIDATION_REQUIRED,-1,--candidate-flag,candidate.csv,gate.csv,comparison.csv,delta.csv,missing_decision.csv,python -m backtester monthly-validate,python -m backtester monthly-compare-validation,Plan only\n",
                encoding="utf-8",
            )
        ...
        followup_checks = [check for check in checks if check.name == "validation_candidate_followup"]
        self.assertEqual(followup_checks[0].status, "BLOCK")
        self.assertIn("unsafe_experiment_id", followup_checks[0].detail)

    def test_validation_candidate_followup_blocks_unsafe_live_risk_note(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            followup = root / "monthly_validation_candidate_followup.csv"
            followup.write_text(
                "priority_rank,experiment_id,status,adoption_status,failed_delta,candidate_validation_args,candidate_scenario_output,candidate_gate_output,comparison_output,delta_output,decision_output,validation_command,comparison_command,risk_note\n"
                "1,unsafe_live,IMPROVED,FULL_VALIDATION_REQUIRED,-1,--candidate-flag,candidate.csv,gate.csv,comparison.csv,delta.csv,missing_decision.csv,python -m backtester monthly-validate,python -m backtester monthly-compare-validation,Try live order after review\n",
                encoding="utf-8",
            )
        ...
        followup_checks = [check for check in checks if check.name == "validation_candidate_followup"]
        self.assertEqual(followup_checks[0].status, "BLOCK")
        self.assertIn("unsafe_risk_note", followup_checks[0].detail)
    ...
    def test_validation_candidate_followup_surfaces_pending_oos_marker(self):
        with TemporaryDirectory() as temp_dir:
            root = Path(temp_dir)
            decision = root / "candidate_decision.csv"
            decision.write_text(
    ...
    def test_validation_candidate_followup_blocks_inconsistent_accepted_decisions(self):
        with TemporaryDirectory() as temp_dir:
            ...
            decision.write_text(
                "candidate_label,comparison_status,decision,decision_reasons,post_cutoff_oos_start_date,post_cutoff_oos_end_date,baseline_failed_required,candidate_failed_required,failed_delta,resolved_count,new_failure_count,unchanged_failure_count,resolved_failure_names,new_failure_names,unchanged_failure_names,new_failure_diagnostics,recommendation\n"
                "manual_accept,REJECT,ACCEPT,oos_review_passed;production_readiness_approved,2026-06-19,2026-06-19,1,1,0,0,1,1,,walk_forward_002,regime_sideways,train_gate_regression=1,inconsistent manual accept.\n",
                encoding="utf-8",
            )
            followup = root / "monthly_validation_candidate_followup.csv"
            followup.write_text(
    ...
    def test_validation_failure_patterns_block_unsafe_live_action(self):
        with TemporaryDirectory() as temp_dir:
            patterns = Path(temp_dir) / "monthly_validation_failure_patterns.csv"
            patterns.write_text(
                "scenario,baseline_failed,baseline_reason,failed_candidate_count,new_failure_candidate_count,resolved_candidate_count,unchanged_failure_candidate_count,candidate_labels_failed,candidate_labels_new_failure,candidate_labels_resolved,candidate_labels_unchanged,dominant_diagnostic,pattern_status,suggested_action,notes\n"
                "walk_001,True,below_excess,3,0,0,3,cash_10,, ,cash_10,same_failure_persists,PERSISTENT_BLOCK,LIVE_ORDER_AFTER_REVIEW,keep paper-only\n",
                encoding="utf-8",
            )
        ...
        pattern_checks = [check for check in checks if check.name == "validation_failure_patterns"]
        self.assertEqual(pattern_checks[0].status, "BLOCK")
        self.assertIn("unsafe_suggested_action", pattern_checks[0].detail)
    ...
    def test_validation_failure_drilldown_blocks_unsafe_live_suggested_action(self):
        with TemporaryDirectory() as temp_dir:
            drilldown = Path(temp_dir) / "monthly_validation_failure_drilldown.csv"
            drilldown.write_text(
                "scenario,category,pattern_status,suggested_action,baseline_reason,likely_root_cause,train_start,train_end,selected_preset,train_excess_return_pct,train_candidate_scores,train_candidate_decision_profiles,train_candidate_direct_scores,train_direct_diagnostics,start,end,baseline_excess_return_pct,baseline_max_drawdown_pct,baseline_trade_count,candidate_count,candidate_labels,candidate_excess_delta_min,candidate_excess_delta_median,candidate_drawdown_delta_median,candidate_trade_delta_median,dominant_diagnostic,evidence_gaps,next_action\n"
                "regime_sideways,regime,PERSISTENT_BLOCK,LIVE_ORDER_AFTER_REVIEW,below_excess,weak_window_return_drag,2024-01-01,2024-12-31,baseline,-1.0,profile_a,decision_a,direct_a,diagnostic_a,2025-01-02,2025-04-17,-7.1,-23.9,10,2,candidate_a,-3.0,-1.5,0.5,2.0,selection_or_exposure_drag,selected_symbols,Run attribution.\n",
                encoding="utf-8",
            )
        ...
        drilldown_checks = [check for check in checks if check.name == "validation_failure_drilldown"]
        self.assertEqual(drilldown_checks[0].status, "BLOCK")
        self.assertIn("unsafe_suggested_action", drilldown_checks[0].detail)
    ...
    def test_accepted_validation_candidate_with_promotion_proof_passes(self):
        with TemporaryDirectory() as temp_dir:
            ...
            decision.write_text(
                "candidate_label,comparison_status,decision,decision_reasons,post_cutoff_oos_start_date,post_cutoff_oos_end_date,baseline_failed_required,candidate_failed_required,failed_delta,resolved_count,new_failure_count,unchanged_failure_count,resolved_failure_names,new_failure_names,unchanged_failure_names,new_failure_diagnostics,recommendation\n"
                "manual_accept,IMPROVED,ACCEPT,oos_review_passed;production_readiness_approved,2026-06-19,2026-06-19,1,0,-1,1,0,0,regime_sideways,,,,manual accept with proof.\n",
                encoding="utf-8",
            )
    ...
    def test_validation_candidate_decision_blocks_unsafe_live_recommendation(self):
        with TemporaryDirectory() as temp_dir:
            decision = Path(temp_dir) / "monthly_validation_candidate_decision.csv"
            decision.write_text(
                "candidate_label,comparison_status,decision,decision_reasons,post_cutoff_oos_start_date,post_cutoff_oos_end_date,baseline_failed_required,candidate_failed_required,failed_delta,resolved_count,new_failure_count,unchanged_failure_count,resolved_failure_names,new_failure_names,unchanged_failure_names,new_failure_diagnostics,recommendation\n"
                "manual_accept,IMPROVED,ACCEPT,oos_review_passed;production_readiness_approved,2026-06-19,2026-06-19,1,0,-1,1,0,0,regime_sideways,,,,place live order after review.\n",
                encoding="utf-8",
            )
        ...
        candidate_checks = [check for check in checks if check.name == "validation_candidate_decision"]
        self.assertEqual(candidate_checks[0].status, "BLOCK")
        self.assertIn("unsafe_recommendation", candidate_checks[0].detail)
    ...
    def test_accepted_validation_candidate_requires_post_cutoff_oos_date(self):
        with TemporaryDirectory() as temp_dir:
            ...
            decision.write_text(
                "candidate_label,comparison_status,decision,decision_reasons,baseline_failed_required,candidate_failed_required,failed_delta,resolved_count,new_failure_count,unchanged_failure_count,resolved_failure_names,new_failure_names,unchanged_failure_names,new_failure_diagnostics,recommendation\n"
                "manual_accept,IMPROVED,ACCEPT,oos_review_passed;production_readiness_approved,1,0,-1,1,0,0,regime_sideways,,,,manual accept with marker-only proof.\n",
                encoding="utf-8",
            )
