...
def main() -> int:
    ...
    register_us_paper_commands(subparsers)
    ...
    monthly_plan_parser.add_argument("--market-beta-proxy-unbuyable-cash-reserve", action="store_true")
    ...
    monthly_backtest_parser.add_argument("--market-beta-proxy-unbuyable-cash-reserve", action="store_true")
    ...
    monthly_attribution_parser.add_argument("--market-beta-proxy-buyable-only", action="store_true")
    monthly_attribution_parser.add_argument("--market-beta-proxy-unbuyable-cash-reserve", action="store_true")
    ...
    monthly_attribution_parser.add_argument("--benchmark-selection-summary-output", default=None)
    ...
    monthly_validate_parser.add_argument("--market-beta-proxy-unbuyable-cash-reserve", action="store_true")
    ...
    monthly_compare_entry_selection_parser = subparsers.add_parser(
        "monthly-compare-entry-selection-rotation",
        help="Compare selected-symbol rotation details for one failed/reference entry month",
    )
    monthly_compare_entry_selection_parser.add_argument("--failed-selection-report", required=True)
    monthly_compare_entry_selection_parser.add_argument("--reference-selection-report", required=True)
    monthly_compare_entry_selection_parser.add_argument("--failed-label", default="failed")
    monthly_compare_entry_selection_parser.add_argument("--reference-label", default="reference")
    monthly_compare_entry_selection_parser.add_argument("--month", required=True)
    monthly_compare_entry_selection_parser.add_argument(
        "--output",
    ...
    monthly_direct_alpha_parser.add_argument(
        ...
        help="Write paper-only per-symbol direct alpha path-drift contribution diagnostics.",
    )
    monthly_direct_alpha_parser.add_argument(
        "--timing-output",
    ...
    monthly_train_decision_parser.add_argument("--market-beta-proxy-unbuyable-cash-reserve", action="store_true")
    ...
    if args.command == "monthly-compare-entry-selection-rotation":
        rows = compare_monthly_entry_selection_rotation_reports(
            _read_csv_dicts(Path(args.failed_selection_report)),
            _read_csv_dicts(Path(args.reference_selection_report)),
            failed_label=args.failed_label,
            reference_label=args.reference_label,
            month=args.month,
        )
        saved = save_monthly_entry_selection_rotation_comparison(rows, args.output)
        reference_only_rows = [
            row for row in rows if row.get("rotation_role") == "reference_only_symbols"
        ]
        failed_only_rows = [
            row for row in rows if row.get("rotation_role") == "failed_only_symbols"
        ]
        reference_winner_rows = [
            row for row in rows if "reference_selected_winner" in str(row.get("diagnostic", ""))
        ]
        print(f"entry_selection_rotation_report  {args.output}")
        print(f"comparison_rows  {saved}")
    ...
    if args.command == "monthly-direct-alpha-diagnostics":
        ...
        path_drift_rows = analyze_monthly_direct_alpha_path_drift(
            symbol_candles,
            cases=cases,
            config=MonthlyRebalanceConfig(
                presets=presets,
        ...
        timing_rows = analyze_monthly_direct_alpha_timing(
            symbol_candles,
            cases=cases,
            config=MonthlyRebalanceConfig(
                presets=presets,
        ...
        print(f"direct_alpha_path_drift_rows  {path_drift_saved}")
    ...
    if args.command == "monthly-attribution":
        ...
        benchmark_selection_rows = (
            ...
            if args.benchmark_selection_output or args.benchmark_selection_summary_output
            else []
        )
        benchmark_selection_summary_rows = (
            analyze_monthly_benchmark_selection_summary(benchmark_selection_rows)
            if args.benchmark_selection_summary_output
            else []
        )
        ...
        if args.benchmark_selection_summary_output:
            save_monthly_benchmark_selection_summary(
                benchmark_selection_summary_rows,
                args.benchmark_selection_summary_output,
            )
        if args.summary_output:
        ...
        if args.benchmark_selection_summary_output:
            print(f"benchmark_selection_summary_rows  {len(benchmark_selection_summary_rows)}")
            print(f"benchmark_selection_summary_report  {args.benchmark_selection_summary_output}")
        if args.stress_drawdown_output:
