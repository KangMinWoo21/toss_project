...
def _validation_remediation_check(path: Path) -> ReadinessCheck:
    ...
    if unsafe_parameter_hints:
        return ReadinessCheck(
            "validation_remediation",
            "BLOCK",
            f"unsafe_parameter_hints={unsafe_parameter_hints[0]}",
        )
...
def _validation_sweep_plan_check(path: Path) -> ReadinessCheck:
    ...
    unsafe_expected_effects = [
        ...
        for row in rows
        if re.search(
            r"\b(live|order|(?<!no-)trade|trading|fetch)\b",
            str(row.get("expected_effect", "")).lower(),
        )
    ]
    if unsafe_expected_effects:
        return ReadinessCheck(
            "validation_sweep_plan",
            "BLOCK",
            f"unsafe_expected_effect={unsafe_expected_effects[0]}",
        )
    first = rows[0]
...
def _validation_sweep_results_check(path: Path) -> ReadinessCheck:
    ...
    unsafe_candidate_args = [
        str(row.get("candidate_validation_args", "")).strip()
        for row in rows
        if re.search(
            r"(^|[^a-z])(live|order|trade|trading|fetch)([^a-z]|$)",
            str(row.get("candidate_validation_args", "")).lower(),
        )
    ]
    if unsafe_candidate_args:
        return ReadinessCheck(
            "validation_sweep_results",
            "BLOCK",
    ...
    unsafe_adoption_statuses = [
        str(row.get("adoption_status", "")).strip()
        for row in rows
        if re.search(
            r"(^|[^a-z])(live|order|trade|trading|fetch)([^a-z]|$)",
            str(row.get("adoption_status", "")).lower(),
        )
    ]
    if unsafe_adoption_statuses:
        return ReadinessCheck(
            "validation_sweep_results",
            "BLOCK",
            f"unsafe_adoption_status={unsafe_adoption_statuses[0]}",
        )
    unsafe_adoption_requirements = [
        ...
        if re.search(
            r"\b(live|order|trade|trading|fetch)\b",
            str(row.get("adoption_requirements", "")).lower(),
        )
    ]
    if unsafe_adoption_requirements:
        return ReadinessCheck(
            "validation_sweep_results",
            "BLOCK",
...
def _validation_candidate_followup_check(path: Path) -> ReadinessCheck:
    ...
    missing_stress_review_output_paths = [
        str(row.get("experiment_id", "")).strip()
        for row in pending_rows
        if "candidate_stress_review_output" in row
        and not str(row.get("candidate_stress_review_output", "")).strip()
    ]
    if missing_stress_review_output_paths:
        unsafe_detail = f"missing_stress_review_output_path={missing_stress_review_output_paths[0]}"
        promotion_blocked.append(unsafe_detail)
        decision_detail += f"; promotion_blocked_decisions={unsafe_detail}"
    ...
    if unsafe_risk_notes:
        unsafe_detail = f"unsafe_risk_note={unsafe_risk_notes[0]}"
        promotion_blocked.append(unsafe_detail)
        decision_detail += f"; promotion_blocked_decisions={unsafe_detail}"
    detail = (
...
def _validation_failure_patterns_check(path: Path) -> ReadinessCheck:
    ...
    if unsafe_actions:
        status = "BLOCK"
    detail = f"{len(rows)} scenarios; statuses: {status_summary}; top={top_detail}{unsafe_detail}"
